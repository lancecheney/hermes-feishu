...
| 分支 | 作用 |
|---|---|
| `main` | 上游 `main` 的纯净镜像，定期 reset 到 `upstream/main`，**不要手动提交** |
...
| `meta` | fork 专属文件（`sync-upstream.sh`、`FEISHU-FORK.md`），同步时一并 rebase 与合入 |
| `feishu` | 集成/默认分支 = `main` + 3 个 `pr/*` + `meta`，**直接 clone 就是这个版本** |

## 使用

```bash
git clone https://github.com/lancecheney/hermes-feishu.git   # 默认分支就是 feishu
# 或显式指定
git clone https://github.com/lancecheney/hermes-feishu.git -b feishu
```
...
2. 把 `main` reset 到 `upstream/main`
3. 把每个 `pr/*` 分支 rebase 到新的 `upstream/main`
4. 重建 `feishu` = `upstream/main` + 所有 `pr/*` 分支，force-push
...
git checkout feishu && git reset --hard upstream/main
git merge --no-edit pr/37847 && git merge --no-edit pr/18188 && git merge --no-edit pr/18131
git push origin feishu pr/37847 pr/18188 pr/18131 --force-with-lease
```
